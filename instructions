...
This file contains the species network. Each line refers to a specific species and includes: i) its name (15 caracters), its charge, its number of elements, and its initial abundance relative to H nuclei. If you add a species, please do not forget to specify the right number of elements and the charge. GRAINOBLE uses this data to check the conservation of mass and charge. Please do not forget to specify the number of species at the beginning of the file. The species network must respect several rules to be read correctly by the fortran code: i) the name of non-porous surface species start with a J and the name of porous surface or bulk species start with a Q. At this moment, one cannot combine the 3-phase and the porosity treatment simultaneously. ii) gas phase species are listed first, then the external surface species, and then the porous or bulk species. iii) The list of gaseous neutral species, surface species, and porous/bulk species need to be identical. iv) Gas phase species that are not considered in surface (e.g. ions) need to be included after the neutral gas phase species.
Example of a species network included 4 neutral species and 2 ions: H D O CO H+ D+ JH JD
JO JCO QH QD QO QCO
...
For gas phase reactions (from 1 to 8), the reaction rates are computed following the methodology adopted in KIDA. A description of the types and formulas can be found \href{hhttp://kida.obs.u-bordeaux1.fr/help}{here}. In addition to the standard gas-phase reactions, GRAINOBLE takes the following processes into account: \ 0: Gas-grain interaction and electron-grain recombination ($Rate(s^{-1}) = A \times \zeta$) (from Flower and Pineau des Forets 2003). 20: Accretion from gas phase to grain surfaces. A=pre-factor constant. 14: Langmuir-Hinshelwood grain-surface chemical reactions. 21: Thermal evaporation. A=pre-factor constant. 22: Cosmic Ray induced general desorption following Hasegawa & Herbst 93. A=branching ratio. 23: Photo-desorption by induced-CR photons + background photons coming from wavelength-dependent experimental studies (see Taquet et al. 2013). A=branching ratio. 26: Photodissociation/photodesorption processes coming from molecular dynamics simulations (see Taquet et al. 2012c). A=branching ratio. 30: Non-porous surace->Pores rate exchange. A=branching ratio. 31: Pores->Non-porous rate following the method introduced by Taquet et al. (2012). A=branching ratio. 32: Surface<->mantle exchange rate following the method introduced by Hasegawa & Herbst (1993).
...
If a spatial evolution has been chosen in input_parameters.in, the folder and the list of files giving the one-dimensional evolution of the physical conditions have to be specified in this file. The format of the input files follows the format given in the examples located in the "phys" folders.
